    ...
    if (!m_id) {
        if (!req.fileValidationError) {
            if (!req.file) {
                ...
                let sql = `INSERT INTO member (m_img, m_username, m_password, m_idcard, m_email, m_active, m_prename, m_firstname, m_lastname, m_race, m_nation, m_religion, m_blood, m_birthday, m_spouse, m_relationship, m_house_no, m_moo, m_alley, m_road, m_subdistrict, m_district, m_province, m_zipcode, m_phone, m_fathername, m_father_occupation, m_mothername, m_mother_occupation,create_at) 
            ...
            VALUES('${req.file.filename}', '${m_username}', '${HashPassword}', '${m_idcard}', '${m_email}', ${m_active}, ${m_prename}, '${m_firstname}', '${m_lastname}',
                ...
                '${m_zipcode}', '${m_phone}', '${m_fathername}', '${m_father_occupation}', '${m_mothername}', '${m_mother_occupation}','${moment().add('year', 543).format()}')`;
                mysqlConnection.query(sql, function (err, result) {
                    if (!err) res.json({
        ...
        mysqlConnection.query(sqlCheck, async function (err, result) {
            if (result.length > 1) {
                ...
                m_spouse = '${m_spouse}', m_relationship = ${m_relationship}, m_house_no = '${m_house_no}', m_moo = '${m_moo}', m_alley = '${m_alley}', m_road = '${m_road}', m_subdistrict = ${m_subdistrict}, m_district = ${m_district}, m_province = ${m_province}, m_zipcode =
                    ...
                    '${m_mothername}', m_mother_occupation = '${m_mother_occupation}', updated_at ='${moment().add('year', 543).format()}' WHERE m_id = ${m_id} `;
                        mysqlConnection.query(sql, function (err, result) {
                            if (!err) res.json({
